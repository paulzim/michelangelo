...
- **Classical ML** (2016–2018) — The initial focus: standardizing feature pipelines, training jobs, and model serving for structured data problems using gradient-boosted trees and traditional ML algorithms.
- **Deep learning at scale** (2019–2021) — As neural networks became central to ranking, NLP, and computer vision use cases, Michelangelo added distributed training infrastructure, GPU support, and a broader range of model architectures.
...
The broader ML community has standardized on open, composable infrastructure: dedicated training frameworks, distributed compute runtimes, and high-performance inference engines. But there is still no dominant open-source end-to-end ML platform — no single place that organizes those building blocks into a coherent, production-ready system.
...
This follows a pattern that has worked before. When large engineering organizations have open sourced solved internal problems — container orchestration, ML frameworks, developer portals — the contributors became the center of ecosystems rather than maintenance burdens fighting ecosystem drift. In each case, open sourcing turned a closed internal system into a community-augmented platform that outlasted the internal team's original investment.

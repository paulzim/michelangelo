...
describe('useResolvedActionItems', () => {
  ...
  it('passes through hierarchy', () => {
    const onSelect = vi.fn();
    const actions: ActionConfig[] = [
      { display: { label: 'A' }, modal: { type: 'custom', component: () => null } },
    ];
    const { result } = renderHook(() => useResolvedActionItems(actions, onSelect));
    expect(result.current[0].hierarchy).toBeUndefined();
  });

...
def _init_kwargs(t, res):
    """Filter res to keys accepted by t.__init__ (handles init=False fields and
    custom __init__ overrides that don't expose every dataclass field)."""
    import inspect

    params = inspect.signature(t.__init__).parameters
    return {k: v for k, v in res.items() if k in params}
...
def ref(value, io: IORegistry):
    ...
    if is_dataclass_instance(value):
        ...
        return type(value)(**_init_kwargs(type(value), res))
